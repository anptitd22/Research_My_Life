---
title: "Dynamic Task Mapping — Airflow 3.1.8 Documentation"
source: "https://airflow.apache.org/docs/apache-airflow/3.1.8/authoring-and-scheduling/dynamic-task-mapping.html"
author:
published:
created: 2026-06-03
description:
...
airflow/example\_dags/example\_dynamic\_task\_mapping.py [\[source\]](https://airflow.apache.org/docs/apache-airflow/3.1.8/_modules/airflow/example_dags/example_dynamic_task_mapping.html)
...
![../_images/mapping_simple_graph.png](https://airflow.apache.org/docs/apache-airflow/3.1.8/_images/mapping_simple_graph.png)

The grid view also provides visibility into your mapped tasks in the details panel:

![../_images/grid_mapped_task.png](https://airflow.apache.org/docs/apache-airflow/3.1.8/_images/grid_mapped_task.png)
...
The `make_list` task runs as a normal task and must return a list or dict (see ), and then the `consumer` task will be called four times, once with each value in the return of `make_list`.
...
airflow/example\_dags/example\_dynamic\_task\_mapping\_with\_no\_taskflow\_operators.py [\[source\]](https://airflow.apache.org/docs/apache-airflow/3.1.8/_modules/airflow/example_dags/example_dynamic_task_mapping_with_no_taskflow_operators.html)
...
1. The callable argument of [`map()`](https://docs.python.org/3/library/functions.html#map "(in Python v3.14)") (`create_copy_kwargs` in the example) **must not** be a task, but a plain Python function. The transformation is as a part of the “pre-processing” of the downstream task (i.e. `copy_files`), not a standalone task in the Dag.
2. The callable always take exactly one positional argument. This function is called for each item in the iterable used for task-mapping, similar to how Python’s built-in [`map()`](https://docs.python.org/3/library/functions.html#map "(in Python v3.14)") works.
...
It is also common to want to combine multiple input sources into one task mapping iterable. This is generally known as “zipping” (like Python’s built-in [`zip()`](https://docs.python.org/3/library/functions.html#zip "(in Python v3.14)") function), and is also performed as pre-processing of the downstream task.
...
Similar to the built-in [`zip()`](https://docs.python.org/3/library/functions.html#zip "(in Python v3.14)"), you can zip an arbitrary number of iterables together to get an iterable of tuples of the positional arguments’ count. By default, the zipped iterable’s length is the same as the shortest of the zipped iterables, with superfluous items dropped. An optional keyword argument `default` can be passed to switch the behavior to match Python’s [`itertools.zip_longest()`](https://docs.python.org/3/library/itertools.html#itertools.zip_longest "(in Python v3.14)") —the zipped iterable will have the same length as the *longest* of the zipped iterables, with missing items filled with the value provided by `default`.
...
This creates one single task to expand against both lists instead. You can `concat` an arbitrary number of iterables together (e.g. `foo.concat(bar, rex)`); alternatively, since the return value is also an XCom reference, the `concat` calls can be chained (e.g. `foo.concat(bar).concat(rex)`) to achieve the same result: one single iterable that concatenates all of them in order, similar to Python’s [`itertools.chain()`](https://docs.python.org/3/library/itertools.html#itertools.chain "(in Python v3.14)").
