...
|write.distribution-mode|not set, see engines for specific defaults, for example [Spark Writes](https://iceberg.apache.org/docs/latest/spark-writes/#writing-distribution-modes)|Defines distribution of write data: none: don't shuffle rows; hash: hash distribute by partition key ; range: range distribute by partition key or sort key if table has an SortOrder|
...
|write.metadata.delete-after-commit.enabled|false|Controls whether to delete the oldest tracked version metadata files after each table commit. See the [Remove old metadata files](https://iceberg.apache.org/docs/latest/maintenance/#remove-old-metadata-files) section for additional details|
...
Thuộc tính bảng dành riêng chỉ được sử dụng để kiểm soát hành vi khi tạo hoặc cập nhật bảng. Giá trị của các thuộc tính này không được lưu giữ như một phần của bảng metadata.
...
Iceberg catalogs hỗ trợ sử dụng thuộc tính catalog để cấu hình hành vi catalog. Dưới đây là danh sách các thuộc tính catalog thường được sử dụng:
...
|metrics-reporter-impl|org.apache.iceberg.metrics.LoggingMetricsReporter|Custom MetricsReporter implementation to use in a catalog. See the [Metrics reporting](https://iceberg.apache.org/docs/latest/metrics-reporting/) section for additional details|

HadoopCatalog và HiveCatalog có thể truy cập các thuộc tính trong các hàm constructors của chúng. Bất kỳ catalog tùy chỉnh nào khác đều có thể truy cập các thuộc tính bằng cách triển khai Catalog.initialize(catalogName, catalogProperties). Các thuộc tính có thể được xây dựng thủ công hoặc được truyền vào từ một công cụ tính toán như Spark hoặc Flink. Spark sử dụng các thuộc tính phiên làm thuộc tính catalog, xem thêm chi tiết trong phần cấu hình Spark. Flink truyền các thuộc tính catalog thông qua câu lệnh CREATE CATALOG, xem thêm chi tiết trong phần Flink.

### Lock catalog properties

Sau đây là các thuộc tính catalog liên quan đến khóa. Chúng được một số triển khai catalog sử dụng để kiểm soát hành vi khóa trong quá trình xác nhận.

|   |   |   |
|---|---|---|
|Property|Default|Description|
|lock-impl|null|a custom implementation of the lock manager, the actual interface depends on the catalog used|
|lock.table|null|an auxiliary table for locking, such as in [AWS DynamoDB lock manager](https://iceberg.apache.org/docs/latest/aws/#dynamodb-lock-manager)|
...
Các thuộc tính sau từ cấu hình Hadoop được sử dụng bởi trình kết nối Hive Metastore. Khóa bảng HMS là một quy trình gồm 2 bước: 

1. Lock Creation: Tạo khóa trong HMS và xếp hàng để thu thập
    
2. Lock Check: Kiểm tra xem khóa đã được lấy thành công chưa
