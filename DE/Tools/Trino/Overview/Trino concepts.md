...
![alt text](images/Tools/Trino/trino_architecture.png)
...
Trino coordinator là máy chủ chịu trách nhiệm phân tích cú pháp các câu lệnh, lập kế hoạch truy vấn và quản lý các node Trino worker. Đây là "bộ não" của một Trino installation và cũng là node mà máy khách kết nối để gửi các câu lệnh thực thi. Mỗi Trino installation phải có một Trino coordinator cùng với một hoặc nhiều Trino worker. Cho mục đích phát triển hoặc thử nghiệm, một phiên bản Trino duy nhất có thể được cấu hình để thực hiện cả hai vai trò. 
...
Khi một tiến trình Trino worker khởi động, nó sẽ tự quảng cáo (advertises / broadcast) tới máy chủ trong coordinator, giúp Trino coordinator có thể thực hiện tác vụ. 
...
Trino là một công cụ truy vấn mà bạn có thể sử dụng để truy vấn nhiều nguồn dữ liệu khác nhau. Chúng bao gồm các kho dữ liệu (data lakes và lakehouses), nhiều hệ thống quản lý cơ sở dữ liệu quan hệ, kho lưu trữ cặp key-value và nhiều kho dữ liệu khác.

[A comprehensive list with more details for each data source is available on the Trino website](https://trino.io/ecosystem/data-source).

Nguồn dữ liệu cung cấp dữ liệu để Trino truy vấn. Cấu hình [catalog](https://trino.io/docs/current/overview/concepts.html#trino-concept-catalog) với Trino [connector](https://trino.io/docs/current/overview/concepts.html#trino-concept-connector) cần thiết cho nguồn dữ liệu cụ thể để truy cập dữ liệu. Với Trino, bạn đã sẵn sàng sử dụng bất kỳ ứng dụng khách nào được hỗ trợ để truy vấn các nguồn dữ liệu bằng SQL và các tính năng của ứng dụng khách đó.

### Connector

Trino có thể truy vấn nhiều loại dữ liệu bằng cách sử dụng một giao diện chung gọi là SPI ([service provider interface (SPI)](https://trino.io/docs/current/develop/spi-overview.html)), cho phép công cụ cốt lõi xử lý các tương tác với từng nguồn dữ liệu như nhau và qua các API tiêu chuẩn. Mỗi đầu nối sau đó phải triển khai SPI, bao gồm việc hiển thị metadata, số liệu thống kê, vị trí dữ liệu và thiết lập một hoặc nhiều kết nối với nguồn dữ liệu cơ bản.

![alt text](images/Tools/Trino/trino_SPI.png)

Mỗi catalog sử dụng một connectorcụ thể. Nếu bạn xem xét tệp cấu hình catalog, bạn sẽ thấy rằng mỗi tệp đều chứa thuộc tính bắt buộc `connector.name` với giá trị xác định connector.
...
Schema là một cách để tổ chức các bảng. Cùng nhau, catalog và schema định nghĩa một tập hợp các bảng và các đối tượng khác có thể được truy vấn. Khi truy cập Hive hoặc cơ sở dữ liệu quan hệ như MySQL bằng Trino, schema được chuyển đổi thành cùng một khái niệm trong cơ sở dữ liệu đích. Các loại trình kết nối khác có thể tổ chức các bảng thành schema theo cách phù hợp với nguồn dữ liệu cơ bản.

### Table

Bảng là một tập hợp các hàng không được sắp xếp, được tổ chức thành các cột có tên và kiểu dữ liệu ([types](https://trino.io/docs/current/language/types.html)). Điều này tương tự như trong bất kỳ cơ sở dữ liệu quan hệ nào. Ánh xạ kiểu dữ liệu từ dữ liệu nguồn sang Trino được xác định bởi connector, khác nhau giữa các connector và được ghi lại trong tài liệu của connector cụ thể, ví dụ như ánh xạ kiểu dữ liệu trong trình kết nối PostgreSQL.

## Query execution model

![alt text](images/Tools/Trino/trino_query_plan.png)

![alt text](images/Tools/Trino/trino_query_plan2.png)

![alt text](images/Tools/Trino/trino_stage_to_task.png)

![alt text](images/Tools/Trino/trino_task.png)

![alt text](images/Tools/Trino/trino_splits.png)
...
Khi Trino phân tích cú pháp một câu lệnh, nó sẽ chuyển đổi câu lệnh đó thành một truy vấn và tạo ra một kế hoạch truy vấn phân tán, sau đó được hiện thực hóa thành một chuỗi các stage được kết nối với nhau chạy trên các Trino worker. Khi bạn truy xuất thông tin về một truy vấn trong Trino, bạn sẽ nhận được snapshot của mọi thành phần tham gia vào việc tạo ra tập kết quả để phản hồi câu lệnh. 

Sự khác biệt giữa câu lệnh (statement) và truy vấn (query) rất đơn giản. Câu lệnh có thể được hiểu là văn bản SQL được truyền đến Trino, trong khi truy vấn đề cập đến cấu hình và các thành phần được khởi tạo để thực thi câu lệnh đó. Truy vấn bao gồm các stages, tasks, splits, connectors và các thành phần và nguồn dữ liệu khác hoạt động phối hợp để tạo ra kết quả.

### Stage

Khi Trino thực thi một truy vấn, nó sẽ chia quá trình thực thi thành một hệ thống phân cấp các stages. Ví dụ: nếu Trino cần tổng hợp dữ liệu từ một tỷ hàng được lưu trữ trong Hive, nó sẽ tạo một stage gốc để tổng hợp đầu ra của nhiều stages khác, tất cả đều được thiết kế để triển khai các phần khác nhau của một kế hoạch truy vấn phân tán. 

Hệ thống phân cấp các stage tạo nên một truy vấn giống như một cây. Mỗi truy vấn đều có một stage gốc, chịu trách nhiệm tổng hợp đầu ra từ các stages khác. Các stages là những gì coordinator sử dụng để mô hình hóa một kế hoạch truy vấn phân tán, nhưng bản thân các stages không chạy trên các Trino workers.

### Task

Như đã đề cập ở phần trước, các stage mô hình hóa một phần cụ thể của kế hoạch truy vấn phân tán, nhưng bản thân các giai đoạn (stages) không thực thi trên các Trino worker. Để hiểu cách một giai đoạn được thực thi, bạn cần hiểu rằng một stage được triển khai như một chuỗi các tác vụ (tasks) được phân bổ trên một mạng lưới các Trino worker.  

Task là "work horse" trong kiến ​​trúc Trino khi một kế hoạch truy vấn phân tán được phân tách thành một chuỗi các stages, sau đó được chuyển đổi thành các nhiệm vụ (task), rồi sau đó thực hiện hoặc xử lý các splits. Một task Trino có đầu vào và đầu ra, và cũng giống như một stage (có thể được thực thi song song bởi một chuỗi các tasks), một task được thực thi song song với một chuỗi các trình điều khiển (drivers).

### Split

Các tác vụ (tasks) hoạt động theo chế độ split, là các phần của một tập dữ liệu lớn hơn. Các stage ở cấp thấp nhất của một kế hoạch truy vấn phân tán sẽ truy xuất dữ liệu thông qua các phần split từ các connector, và các stage trung gian ở cấp cao hơn của một kế hoạch truy vấn phân tán sẽ truy xuất dữ liệu từ các stages khác. 

Khi Trino lập lịch truy vấn, coordinator sẽ truy vấn một connector để lấy danh sách tất cả các split khả dụng cho một bảng. Coordinator sẽ theo dõi máy nào đang chạy tasks nào và split nào đang được xử lý bởi tasks nào.
...
Mỗi task chứa một hoặc nhiều trình điều khiển (driver) song song. Các trình điều khiển này tác động lên dữ liệu và kết hợp các toán tử (operator) để tạo ra đầu ra, sau đó được tổng hợp bởi một task và chuyển đến một task khác ở một stage khác. Trình điều khiển là một chuỗi các thể hiện toán tử, hoặc bạn có thể hình dung trình điều khiển như một tập hợp các toán tử vật lý trong bộ nhớ. Đây là mức độ song song thấp nhất trong kiến ​​trúc Trino. Một trình điều khiển có một đầu vào và một đầu ra.

### Operator

Toán tử sử dụng, biến đổi và tạo ra dữ liệu. Ví dụ: quét bảng sẽ lấy dữ liệu từ một connector và tạo ra dữ liệu có thể được sử dụng bởi các toán tử khác, còn toán tử lọc sẽ sử dụng dữ liệu và tạo ra một tập hợp con bằng cách áp dụng một vị từ lên dữ liệu đầu vào.  

### Exchange

Trao đổi dữ liệu giữa các node Trino cho các stages khác nhau của truy vấn. Các task tạo dữ liệu vào bộ đệm đầu ra và sử dụng dữ liệu từ các tasks khác bằng exchange client.